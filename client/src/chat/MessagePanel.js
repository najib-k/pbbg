...
export default function MessagePanel(props) {
    ...
    if (channel?.messages) {
        ...
        list = messages.map((m, index) => <Message key={m.id} id={m.id} senderName={m.player.name} text={m.text} bgcolor={(index % 2) ? primary.msgBgColor : secondary.msgBgColor}/>);
    }
            ...
            <div className="messages-panel" style={{}}>
                {channel &&
                    <div className="messages-input">
                        ...
                        <input type="text" onChange={inputHandler} value={input_value} label="message" />
